...
async function loadIgnorePatterns(configPath) {
  ...
  try {
    ...
    if (error &&
      (error.code === "ERR_MODULE_NOT_FOUND" || /Cannot find package/i.test(String(error.message || "")))) {
      console.warn(
        `Fallback lint: unable to load ESLint config at ${absolutePath}. Continuing without ignore patterns.`
      );
      return [];
    }
    throw error;

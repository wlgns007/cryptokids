...
app.post("/earn", requireAdminKey, (req, res) => {
  const { userId, amount, task } = req.body;
...
app.post("/shop/mintSpend", (req, res) => {
  ...
  if (!userId || !rewardId) return res.status(400).json({ error: "Missing userId/rewardId" });

  const reward = db.prepare(`SELECT id, name, price FROM rewards WHERE id = ? AND active = 1`).get(Number(rewardId));
  if (!reward) return res.status(404).json({ error: "REWARD_NOT_FOUND" });

  // ADD THIS GUARD (prevent useless QR when balance is too low)
  const current = getBalance(String(userId));
  ...
  res.json({ url, expiresAt: payload.exp, payload: { userId: payload.userId, item: reward.name, price: reward.price } });
});
...
app.post("/spend", requireAdminKey, (req, res) => {
  try {
    ...
    const next = applyDelta(String(userId), -Math.floor(amt), item || "", "spend", null, null);
...
// History for child view  ⬇⬇  (put this ABOVE the "/" handler)
...
// REPLACE this entire route with the version below
app.get('/history.csv/:userId', (req, res) => {
  const userId = req.params.userId?.trim();
  if (!userId) return res.status(400).send('userId required');

  // Your ledger schema: (user_id, delta, reason, kind, ... created_at)
  const rows = db.prepare(
  ...
  const csv = rows.map(r => {
    ...
    // amount = absolute value of delta, date = ISO from created_at (seconds)
    const amount = Math.abs(Number(r.delta));
...
app.post("/admin/credit", requireAdminKey, (req, res) => {
  try {
    ...
    const next = applyDelta(String(userId), Math.floor(amt), reason || "", "earn", null, { via: "admin-credit" });
    res.json({ balance: next });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
...
// ===== start =====
app.listen(PORT, () => {
  console.log(`Parents Shop API listening on http://localhost:${PORT}`);
});
